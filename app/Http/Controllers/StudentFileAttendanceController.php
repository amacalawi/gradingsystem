...
class StudentFileAttendanceController extends Controller
{
    ...
    {   
        ...
        $segment = request()->segment(5);
        $types = (new EducationType)->all_education_types();
        $student_id = AttendanceSheets::join('students', 'students.user_id', '=', 'attendance_sheets.user_id')->where('attendance_sheets.id', $id )->first(['students.id']);
        $student = (new Student)->fetch( $student_id->id );
    ...
    {   
        ...
        $student_dtl = Student::where('identification_no', $student_arr[0] )->first(['id', 'user_id', 'role_id']);

        $status = $this->get_status_via_auth_user_type(Auth::user()->id, $student_dtl->role_id);
    ...
    {   
        ...
        $student_dtl = Student::where('identification_no', $staff_arr[0] )->first(['id', 'user_id', 'role_id']);
        $status = $this->get_status_via_auth_user_type(Auth::user()->id, $student_dtl->role_id);

        $attendancesheets->user_id = $student_dtl->user_id;
        $attendancesheets->role_id = $student_dtl->role_id;
