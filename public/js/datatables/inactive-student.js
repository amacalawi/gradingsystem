...
var DatatableDataLocalDemo = function () {
	//== Private functions
 	console.log(base_url + 'memberships/students/all-inactive');
	...
	var datatable = $('.m_datatable').mDatatable({
		...
		data: {
			...
			source: {
			  read: {
				// sample GET method
				method: 'GET',
				url: base_url + 'memberships/students/all-inactive',
				map: function(raw) {
				  // sample data mapping
				  var dataSet = raw;
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				if ($privileges[3] == 1) {
					return '\
						<a data-row-id="' + row.studentID + '" action="Active" href="javascript:;" class="toggle-status m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="activate this">\
