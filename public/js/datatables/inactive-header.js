...
var DatatableDataLocalDemo = function () {
	//== Private functions
 	console.log(base_url + 'components/menus/headers/all-inactive');
	...
	var datatable = $('.m_datatable').mDatatable({
		...
		data: {
			...
			source: {
			  read: {
				// sample GET method
				method: 'GET',
				url: base_url + 'components/menus/headers/all-inactive',
				map: function(raw) {
				  // sample data mapping
				  var dataSet = raw;
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				if ($privileges[3] == 1) {
					return '\
						<a data-row-id="' + row.headerID + '" action="Active" href="javascript:;" class="toggle-status m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="activate this">\
...
jQuery(document).ready(function () {
	...
	$body.on('click', '.toggle-status', function (e){
		...
		var $url = base_url + 'components/menus/headers/update-status/' + $rowID;
