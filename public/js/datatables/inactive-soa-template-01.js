...
var DatatableDataLocalDemo = function () {
	//== Private functions
 	console.log(base_url + 'components/csv-management/soa-template-01/all-inactive');
	...
	var datatable = $('.m_datatable').mDatatable({
		...
		data: {
			...
			source: {
			  read: {
				// sample GET method
				method: 'GET',
				url: base_url + 'components/csv-management/soa-template-01/all-inactive',
				map: function(raw) {
				  // sample data mapping
				  var dataSet = raw;
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
                    <a data-row-id="' + row.soa_templateID  + '" action="Active" href="javascript:;" class="toggle-status m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="activate this">\
