!function($) {
    ...
    batch.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    batch.prototype.required_fields = function() {
    ...
    batch.prototype.price_separator = function (input) {
        var output = input
        if (parseFloat(input)) {
    ...
    batch.prototype.do_uploads = function($id) {
    ...
    {   
        ...
        */
        ...
        this.$body.on('click', '.submit-btn', function (e){
            e.preventDefault();
            var $self = $(this);
            var $form = $('form[name="batch_form"]');
            var $error = $.batch.validate($form, 0);
                                ...
                                swal({
                                    ...
                                    onClose: () => {
                                        if ($form.find("input[name='method']").val() == 'add') {
                                            window.location.replace(base_url + 'components/schools/schoolyears');
                                        }
                                    }
                                });
    ...
    //init batch
    $.batch = new batch, $.batch.Constructor = batch
