        ...
        <tbody>
            <tr>
                ...
                @foreach ($components as $component)
                    <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 component-header" colspan="{{ floatval($component->columns) - floatval(1) }}" data-toggle="m-tooltip" data-placement="bottom" title="{{ $component->description }}">{{ $component->name }}</th>
                @endforeach
            ...
            <tr>
                ...
                @foreach ($components as $component)
                    ...
                    @endif
                    <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 hidden" title="quarterly percentage">{{ $component->percentage }}%</th>
            ...
            <tr class="tr_shaded">
                <td class="shaded fixed freeze text-center scrolling_table_1"><strong>-</strong></td>
                ...
                @foreach ($components as $component)
                    ...
                    @endif
                    <td group="{{ $component->id }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 percentage-header hidden">&nbsp;</td>
                @endforeach
                <td class="shaded fixed freeze_vertical text-center scrolling_table_1 hidden">&nbsp;</td>
            ...
            @if (count($male_students) > 0)
                <tr class="tr_shaded">
                    ...
                    @foreach ($components as $component)
                        @foreach ($component->activities as $activity)
                            <th group="{{ $component->id }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 activity-cell-header gray-bg">{{ $activity->value }}</th>
                        @endforeach
                        @if ($component->is_sum_cell > 0)
                            <th group="{{ $component->id }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 sum-header gray-bg">{{ $component->sum_value }}</th>
                        @endif
                        @if ($component->is_hps_cell > 0)
                            <th group="{{ $component->id }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 hps-header gray-bg">{{ $component->sum_value }}</th>
                        @endif
                        @if ($component->is_ps_cell > 0)
                            <th group="{{ $component->id }}" maxvalue="{{ $component->sum_value }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 ps-header gray-bg">100%</th>
                        @endif
                        <td group="{{ $component->id }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 percentage-header hidden gray-bg">&nbsp;</td>
                    @endforeach
                    <td class="fixed freeze_vertical text-center scrolling_table_1 hidden">&nbsp;</td>
                ...
                @foreach ($male_students as $student)
                    <tr class="">
                        ...
                        @foreach ($components as $component)
                            ...
                            @foreach ($component->activities as $activity)
                                ...
                                <td group="{{ $component->id }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 no-padding">
                                    <input {{ ($grading->locked > 0) ? 'disabled="disabled"' : '' }} value="{{ $activity->id.'_'.$student->student_id }}" name="activity[]" class="hidden text-cell" {{ ($segment == 'edit') ? '' : 'disabled="disabled"'}} type="text"/>    
                                    <input {{ ($grading->locked > 0) ? 'disabled="disabled"' : '' }} maxvalue="{{ $activity->value }}" value="{{ $score }}" name="score[]" class="numeric-double activity-cell text-cell" {{ ($segment == 'edit') ? '' : 'disabled="disabled"'}} type="text"/>
                                </td>
                            @endforeach
                            @if ($component->is_sum_cell > 0)
                            ...
                            @endif
                            <td group="{{ $component->id }}" maxvalue="{{ $component->percentage }}" class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1 percentage-cell hidden">
                                @php 
                                    if (floatval($sum) > 0) {
                        ...
                        <td class="shaded fixed freeze_vertical text-center scrolling_table_1 no-padding tc-cell hidden">
                            ...
                            <input {{ ($grading->locked > 0) ? 'disabled="disabled"' : '' }} name="rating[]" value="{{ $gradings->get_colum_via_gradingsheet_student('rating', $grading->id, $student->student_id) }}" class="text-cell" {{ ($segment == 'edit') ? '' : 'disabled="disabled"'}} type="text"/>
                            <input {{ ($grading->locked > 0) ? 'disabled="disabled"' : '' }} name="ranking[]" value="{{ $gradings->get_colum_via_gradingsheet_student('ranking', $grading->id, $student->student_id) }}" class="text-cell" {{ ($segment == 'edit') ? '' : 'disabled="disabled"'}} type="text"/>
                        </td>
            ...
            @if (count($female_students) > 0)
                <tr class="tr_shaded">
                    ...
                    @foreach ($components as $component)
                        @foreach ($component->activities as $activity)
                            <th group="{{ $component->id }}" class="fixed freeze_vertical text-center scrolling_table_1 activity-cell-header gray-bg">&nbsp;</th>
                        @endforeach
                        @if ($component->is_sum_cell > 0)
                            <th group="{{ $component->id }}" class="fixed freeze_vertical text-center scrolling_table_1 sum-header gray-bg">&nbsp;</th>
                        @endif
                        @if ($component->is_hps_cell > 0)
                            <th group="{{ $component->id }}" class="fixed freeze_vertical text-center scrolling_table_1 hps-header gray-bg">&nbsp;</th>
                        @endif
                        @if ($component->is_ps_cell > 0)
                            <th group="{{ $component->id }}" maxvalue="{{ $component->sum_value }}" class="fixed freeze_vertical text-center scrolling_table_1 ps-header gray-bg">&nbsp;</th>
                        @endif
                        <td group="{{ $component->id }}" class="fixed freeze_vertical text-center scrolling_table_1 percentage-header hidden gray-bg">&nbsp;</td>
                    @endforeach
                    <td class="fixed freeze_vertical text-center scrolling_table_1 hidden">&nbsp;</td>
                    <td class="fixed freeze_vertical text-center scrolling_table_1 gray-bg">&nbsp;</td>
                    <td class="fixed freeze_vertical text-center scrolling_table_1 gray-bg hidden">&nbsp;</td>
