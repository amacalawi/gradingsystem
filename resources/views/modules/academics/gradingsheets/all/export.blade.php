    ...
    <tbody>
        ...
        <tr>
            <th class="shaded fixed freeze text-center scrolling_table_1 cities">
                Level and Section:{{ $grading->level_name }} ({{ $grading->section_name }})
            </th>
        </tr>
        <tr>
            <th class="shaded fixed freeze text-center scrolling_table_1">
                Subject and Teacher:{{ $grading->subject_name }} ({{ $grading->teacher }})
            </th>
        </tr>
        <tr>
            <th class="shaded fixed freeze text-center scrolling_table_1">
                Batch and Quarter:{{ $grading->batch_name }} ({{ $grading->quarter_name }})
            </th>
        </tr>
        <tr>
            <th class="shaded fixed freeze text-center scrolling_table_1">
                Adviser:{{ $grading->adviser }}
            </th>
        </tr> 
        <tr>
            <td></td>
        ...
        <tr>
            <th class="shaded fixed freeze text-center scrolling_table_1">1</th> @php $column_letter_repo['#'] = $column_letter++; @endphp
            <th class="shaded fixed freeze text-center scrolling_table_1">&nbsp;</th> @php $column_letter_repo['student'] = $column_letter++; @endphp
            @php $component_percentage = 0; @endphp
            
            @foreach ($components as $component)
                @php $iteration = 1; @endphp
                @foreach ($component->activities as $activity)
                    <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1" title="{{ $activity->activity }}">A{{ $iteration }}</th> @php $column_letter_repo['A'.$column_letter] =  $column_letter++; @endphp
                    @php $iteration++; @endphp
                @endforeach
                <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1" title="total activities">SUM</th> @php $column_letter_repo['SUM'.$column_letter] =  $column_letter++; @endphp
                <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1" title="highest possible score">HPS</th>@php $column_letter_repo['HPS'.$column_letter] =  $column_letter++; @endphp
                <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1" title="percentage score">PS</th>@php $column_letter_repo['PS'.$column_letter] =  $column_letter++; @endphp
                <th class="{{ $component->palette }} fixed freeze_vertical text-center scrolling_table_1" title="quarterly percentage">{{ $component->percentage }}%</th> @php $column_letter_repo['percent'.$column_letter] =  $column_letter++; @endphp
                @php $component_percentage += $component->percentage; @endphp

            @endforeach
            <th class="shaded fixed freeze_vertical text-center scrolling_table_1" title="total quarterly percentage">{{ $component_percentage }}%</th>@php $column_letter_repo['initial'] =  $column_letter++; @endphp
            <th class="shaded fixed freeze_vertical text-center scrolling_table_1 quarter-bg">&nbsp;</th>@php $column_letter_repo['QG'] =  $column_letter++; @endphp
            <th class="shaded fixed freeze_vertical text-center scrolling_table_1">&nbsp;</th>@php $column_letter_repo['TC'] =  $column_letter++; @endphp
        </tr>
        <tr class="tr_shaded">
